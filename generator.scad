...
function stone_path_polygon(negate) = rotate90arr([
    ...
    [negate*neck_width/2, 0, 0],
    [negate*neck_width/2, neck_height, 0],
    [negate*necktie_width/2, neck_height, 0],
]);
...
module rail(path, control_points, metals, backplates, ties, stone_path, angle, straight = true) {
    // color([1, 0, 0]) spline_wall(path, width=0.1, height=0.01, subdivisions=0, loop=!straight);
...
module draw_rail(metals, backplates, ties, stone_path) {
    ...
    scale([scale_factor, scale_factor, 0.15]) {
        ...
        scale([1, foreshortening_factor, 1]) {
            ...
            translate([0, 4, 0])
                rotate([0, 0, -45])
                    rail(diagonal_path, diagonal_control_points, metals, backplates, ties, stone_path, 90, straight = true);
            translate([0, -4, 0])
                rotate([0, 0, 45])
                    rail(diagonal_path, diagonal_control_points, metals, backplates, ties, stone_path, 90, straight = true);
        }
